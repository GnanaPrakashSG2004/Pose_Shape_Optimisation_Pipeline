function shape_optimized_wireframe_img_coords = ceresShapeOptimizer(seq, frm, id, label_dir, kpNetOutputFile, kpLookupFile, mean_shape_file, def_vector_file)
  % label_dir directory path must be relative to './devkit/matlab' directory
    ...
    [~, transMatrix, rotMatrix] = ceresPoseOptimizer(seq, frm, id, label_dir, kpNetOutputFile, kpLookupFile, mean_shape_file, def_vector_file);
    ...
    shape_optimized_wireframe = zeros(14, 3, numel(seq));
    shape_optimized_wireframe_img_coords = zeros(14, 2, numel(seq));
    ...
    for i=1:numel(seq)
      ceresInputID = fopen("ceres/ceres_input_singleViewShapeAdjuster.txt", "w");
        ...
        for j=1:3
          fprintf(ceresInputID, "%f ", rotMatrix(j, :)); fprintf(ceresInputID, "\n");
        end
        fprintf(ceresInputID, "%f ", transMatrix(:)); fprintf(ceresInputID, "\n");
      fclose(ceresInputID);
  
      cd ceres/;
      system("./singleViewShapeAdjuster");
      optimizerOutput = importdata("ceres_output_singleViewShapeAdjuster.txt");
      cd ../;
      
      new_rotMatrix = optimizerOutput(1:3, :);
      new_transMatrix = optimizerOutput(4, :);
      optimized_wireframe = (optimizerOutput(5:end, :) * new_rotMatrix') + new_transMatrix;
      shape_optimized_wireframe(:, :, i) = optimized_wireframe * K';
      shape_optimized_wireframe_img_coords(:, :, i) = [shape_optimized_wireframe(:, 1) ./ shape_optimized_wireframe(:, 3), shape_optimized_wireframe(:, 2) ./ shape_optimized_wireframe(:, 3)];
    end
  end